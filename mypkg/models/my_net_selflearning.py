...
# compared with my_net.py or my_net_multi.py, in this file I train the model via aiming to reconstruct its input.
import numpy as np
import torch
...
class myNet(nn.Module):
    def __init__(self, config):
        ...
        self.fc_init_t = nn.Linear(config.block_size, config.ndim0) # the fc layer for time axis
        #self.sp_attn = spAttention(config)
        self.transformer = nn.ModuleDict(dict(
            fc_init = nn.Linear(config.ndim0, config.ndim),
            drop = nn.Dropout(config.dropout),
            h = nn.ModuleList([Block(config) for _ in range(config.n_layer)]),
            ln_f = LayerNorm(config),
        ))
        self.fc_out = nn.Linear(config.ndim, config.target_dim, bias=True)
    ...
    def forward(self, inputs):
        ...
        # this part is from [wei2024(annot. at p. 4)](zotero://open-pdf/library/items/LE6RZHGM?page=4&annotation=LGP5F38L) 
        # it can be regarded as an attention on the feature axis
        # the weights, with this step, num_features do not matter
        alphas = self.fc_init_t(inputs.transpose(1, 2)) # batchsize x num_features x ndim0 (p)
        # for each num_features vectors, normalize with exp
        alphas = alphas/torch.exp(alphas).sum(dim=-2, keepdim=True)
        x = inputs @ alphas # batchsize x len_seq x ndim0 (p)



        # forward the GPT model itself
        # since the last part is also a spatial attention, I do not use self.sp_attn here
        ...
        rec_x = self.fc_out(x) # batchsize x len_seq x num_features
        return rec_x
