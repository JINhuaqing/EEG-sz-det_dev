...
# compare with my_main_model_dis_base.py, I use all time points to predict the seizure label.
import numpy as np
import torch
...
class myNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.aux_loss:
            ...
            self.fc_cut = nn.Linear(config.target_dim, config.nfeature, bias=True)
    ...
    def forward(self, inputs):
        ...
        if self.config.aux_loss:
            return probs_aux, log_probs_cls
        else:
