...
class myNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_prbs = nn.Linear(config.nfeature, config.nfeature*2**config.k, bias=False)
    ...
    def forward(self, inputs, inputs_raw):
        ...
        # below, the code can be changed for different target
        # to model, X_t+dlt =  Xt + dlt * A * Xt
        dlt = 1/self.config.fs
        dims = x.shape[:-1]
        x = x.view(*dims, self.config.nfeature, -1)
        x = torch.matmul(x, inputs_raw.unsqueeze(-1)).squeeze(-1)
        x = dlt * (x + inputs_raw) # It is wrong
        #x = dlt * x + inputs_raw # It is correct
        
        # to probs, 19 -> 19 x 2^k
        x = self.fc_prbs(x)
        dims = x.shape[:-1]
        x = x.view(*dims, -1, 2**self.config.k)
        x = self.softmax(x)
        return x
